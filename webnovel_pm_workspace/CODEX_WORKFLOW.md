...
- 원고 작성 전에는 `story_bible`, `character_bible`, `ability_rules`, `canon_log`, `timeline`, `foreshadowing_ledger`, `style_guide`, `rolling_context` 역할의 문서를 확인합니다. 장기 연재 또는 새 인물 추가가 포함되면 `CAST_HARNESS`와 `LONGFORM_HARNESS`도 확인합니다. 외부 자료, 플랫폼 공개, 민감 소재, 실존 유사성, 패턴 반복 우려가 있으면 rights/platform/content/human contribution/pattern harness도 확인합니다.
...
## 7-3. 권리/플랫폼/콘텐츠 리스크 점검 절차

1. 외부 자료, 표지, 외주, 공동작업이 있으면 `rights_log.md`에 기록합니다.
2. 사용자가 직접 선택, 수정, 승인한 창작 기여는 `human_contribution_log.md`에 기록합니다.
3. 공개 또는 투고 전에는 `platform_policy_check.md`와 `ai_usage_disclosure_note.md`를 확인합니다.
4. 폭력성, 선정성, 혐오 표현, 실존 유사성, 민감 묘사는 `content_risk_check.md`, `real_entity_risk_check.md`, `sensitivity_check.md`에 기록합니다.
5. 정책 또는 권리 상태가 불명확하면 공개용 packaging이나 final 확정을 보류하고 사용자 확인 필요 사항으로 보고합니다.

## 7-4. 패턴 반복 및 적대 세력 단계 점검 절차

1. 각 회차 후 `episode_pattern_log.json`에 opening, conflict, reward, cliffhanger pattern을 기록합니다.
2. 같은 패턴이 반복되면 다음 회차 outline에서 변주 계획을 먼저 작성합니다.
3. 적대 세력 압박은 `opposition_ladder.json`의 현재 단계와 unlock 조건을 확인합니다.
4. 적대 세력이나 능력 압박이 갑자기 과도해지면 final 저장 전 recovery_plan 또는 canon_change_request 필요 여부를 검토합니다.
