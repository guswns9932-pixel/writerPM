...
승인된 개요를 바탕으로 한국 웹소설 모바일 연재 감성의 회차 초안을 작성한다.
...
초안 작성 시 STYLE_HARNESS.md의 다음 규칙을 **기본값**으로 적용:

### 초안 단계에서 적용할 규칙
1. **기계적 단문 반복 금지** - 같은 구조 반복 3회 이상 금지
2. **의미 라벨링 금지** - "이것이 첫 선택이다", "약함의 선택" 같은 표현 금지
3. **추상어 남발 금지** - 추상어는 반드시 구체적 장면과 연결
4. **감각 묘사 통합** - 설명이 아니라 경험으로 표현
5. **감정 행동화** - 감정 선언 대신 신체 반응 표현
6. **대사 전후 자연스러운 반응** - 대사 고립 금지
7. **전투 장면의 목표/압박/대가** - 동작 나열이 아닌 갈등 중심
8. **의인화 표현 금지** - 신체 부위/물체가 주체처럼 행동하는 표현 금지
9. **능력 표현 신체화** - 능력의 충동/효과는 신체 변화로만 표현, 추상적 설명 금지

### 초안 작성 체크리스트
- [ ] 같은 문장 구조가 3회 이상 반복되는 구간이 있는가?
- [ ] 의미 라벨링 문장이 있는가?
- [ ] 감각 묘사가 설명문처럼 분리되어 있는가?
- [ ] 감정이 선언되어 있는가?
- [ ] 대사가 고립되어 있는가?
- [ ] 전투 장면에 목표/압박이 명확한가?
- [ ] 신체 부위/물체를 의인화한 문장이 있는가?
- [ ] 능력 효과를 추상적으로 설명한 문장이 있는가?

### READABILITY_HARNESS / KOREAN_GRAMMAR_HARNESS 적용

초안은 문장 흐름만 STYLE_HARNESS로 검사하고 끝내지 않는다. 저장 전 다음도 함께 확인한다.

- `READABILITY_HARNESS.md`: 중학생 수준 가독성 기준(어휘 수준, 문장 길이, 문체 일관성)
- `KOREAN_GRAMMAR_HARNESS.md`: 문피아 스타일 한국어 문법(주어 생략, 조사, 종결사, 대화체/문어체 구분)
