...
검수 결과와 사용자 지시를 바탕으로 허용된 범위 안에서 수정본을 작성한다.

## 입력 파일

- 수정 대상 draft 또는 final 후보
- `review 파일`
- `revision_note.md`
- `user_feedback_log`
- `feedback_application_plan 필요 시`

## 출력 파일

- `episode_XXX_revision_vN.md`
...
## STYLE_HARNESS 적용 규칙 (문체 수정)

문체 수정이 필요한 경우 STYLE_HARNESS.md의 **15번 원고 수정 순서**를 따름:

### 수정 순서 (반드시 이 순서대로)
1. **AI식 문장 제거** - 의미 라벨링, 장면 제목, 메타 설명 제거
2. **기계적 단문 리듬 완화** - 문장 길이와 종결 다양화
3. **감각 묘사를 장면 안에 통합** - 설명 → 경험
4. **감정 선언을 행동 반응으로 변환** - 표현 → 반응
5. **의인화 표현 제거** - 신체 부위/물체 의인화 → 구체 신체 반응
6. **능력 표현 신체화** - 추상 설명 → 신체 변화
7. **전투 장면에 목표/압박/대가 추가** - 나열 → 갈등
8. **대사 전후 흐름 보강** - 고립 대사 제거
9. **최종 원고에서 작업용 제목과 메타 설명 제거**
10. **리듬 검사** (QUALITY_HARNESS 문장 리듬 검사 항목 전수 점검, READABILITY_HARNESS 가독성 기준 확인, KOREAN_GRAMMAR_HARNESS 문법/문피아 스타일 확인)
